...
Builds on GeneralUnified.lean to show that Nash equilibria at level k can be
...
def embedPureProfile {V₀ V₁ : I → Type*}
    (emb : ∀ i, V₀ i → V₁ i) (p : GeneralUnified.PureProfile I V₀) :
    GeneralUnified.PureProfile I V₁ :=
  fun i => emb i (p i)
...
def embedProfile {V₀ V₁ : I → Type*}
    ...
    (σ : GeneralUnified.Profile I V₀) : GeneralUnified.Profile I V₁ :=
...
structure GeneralSignTower (I : Type*) [DecidableEq I] [Fintype I] where
  ...
  game : (k : ℕ) → GeneralUnified.SignGame I (V k)
  ...
  coherent : ∀ k i (p : GeneralUnified.PureProfile I (V k)) (a b : V k i),
    (game (k+1)).sign i (embedPureProfile (embed k) p) (embed k i a) (embed k i b)
    = (game k).sign i p a b
  playerConvex_left : ∀ k i (p : GeneralUnified.PureProfile I (V k))
    (a b c x : V k i),
    (betw k i).between c a b →
    ((game k).sign i p a x).nonneg →
    ((game k).sign i p b x).nonneg →
    ((game k).sign i p c x).nonneg
  playerConvex_right : ∀ k i (p : GeneralUnified.PureProfile I (V k))
    (a b c x : V k i),
    (betw k i).between c a b →
  ...
  opponentConvex : ∀ k i (j : I) (_hj : j ≠ i)
    (p q : GeneralUnified.PureProfile I (V k)) (a b : V k i),
    (∀ m, m ≠ j → p m = q m) →
    ∀ (r : GeneralUnified.PureProfile I (V k)),
    (∀ m, m ≠ j → r m = p m) →
...
noncomputable def convCloseProfile (k : ℕ)
    (σ : GeneralUnified.Profile I (T.V k)) :
    GeneralUnified.Profile I (T.V k) :=
...
theorem DevFaceLE_update_self (k : ℕ) {i : I}
    {σ : GeneralUnified.Profile I (T.V k)} {F : Face (T.V k i)}
    {A B : Face (T.V k i)} :
...
theorem DevFaceLE_convClosure_left (k : ℕ) {i : I}
    {σ : GeneralUnified.Profile I (T.V k)} {A B : Face (T.V k i)}
...
theorem DevFaceLE_convCloseProfile (k : ℕ) {i : I}
  ...
  set τ : Finset I → GeneralUnified.Profile I (T.V k) :=
...
theorem DevFaceLE_embed (k : ℕ) {i : I}
  ...
  classical
  set q : GeneralUnified.PureProfile I (T.V k) :=
...
def ProfileRefines (k : ℕ)
    (σ' : GeneralUnified.Profile I (T.V (k+1)))
    (σ : GeneralUnified.Profile I (T.V k)) : Prop :=
...
def IsConvexClosed (k : ℕ) (σ : GeneralUnified.Profile I (T.V k)) : Prop :=
